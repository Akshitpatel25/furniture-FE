...
export default function SupplierDashboard() {
  ...
  const getStats = () => {
    ...
    const totalRevenue = products.reduce((sum: number, p: Product) => sum + (p.price * p.stock), 0);
    const totalStock = products.reduce((sum: number, p: Product) => sum + p.stock, 0);
    
    return [
      { title: "Total Products", value: totalProducts.toString(), change: "products added", icon: Package },
      { title: "Revenue", value: `$${totalRevenue.toLocaleString()}`, change: "from inventory", icon: DollarSign },
      { title: "Stock Value", value: totalStock.toString(), change: "items in stock", icon: TrendingUp },
    ];
  };
